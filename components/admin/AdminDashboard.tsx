      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="glass-premium border-b border-cs-violet/20 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Link href="/" scroll={false} className="text-xs uppercase tracking-widest text-cs-silver/60 hover:text-cs-neon px-3 py-2">
              View Site
            </Link>
          ...
          <div className="space-y-4 max-w-xl">
            ...
            <div className="glass-premium rounded-xl p-5 space-y-4 stroke-purple max-w-2xl">
              ...
              <p className="admin-file-tree text-cs-neon break-all">
                {hero.videoUrl ?? "/content/hero/hero.mp4"}
        ...
        {tab === "Portfolio" && (
          <div className="space-y-4 max-w-2xl">
            {portfolio.map((p, i) => (
              <div key={p.id} className="surface rounded-xl p-4 grid gap-2 sm:grid-cols-2 border border-white/5">
                ...
                <AdminInput placeholder="Image path (/content/...)" value={p.image ?? ""} onChange={(e) => { const n = [...portfolio]; n[i] = { ...p, image: e.target.value }; setPortfolio(n); }} />
            ...
            <SaveBtn onClick={() => { setPortfolio(portfolio); alert("Portfolio saved."); }} />
          </div>
        )}
        ...
        {tab === "Gallery" && (
          <div className="space-y-4 max-w-2xl">
            ...
            <p className="text-xs text-cs-silver/45">Add image paths under /public/content/gallery/ — lazy-loaded on site.</p>
            {galleryItems.map((img, i) => (
              <div key={img.id} className="surface rounded-xl p-3 grid gap-2 sm:grid-cols-2 border border-white/5">
